...
function FormFacade(data)
{
    ...
    this.fetchScrape = function(publishId)
    {
        if(this.data.request.query.template == 'cloud')
            ...
            return fetch(`https://formfacade-template-default-rtdb.firebaseio.com/scrape/${publishId}-${this.data.request.query.uid}.json`).then(req=>req.json());
        return fetch('https://formFacade.firebaseio.com/scrape/'+publishId+'.json').then(req=>req.json());
    }
    ...
    this.fetchPublish = function(publishId)
    {
        if(this.data.request.query.template == 'cloud')
            ...
            return fetch(`https://formfacade-template-default-rtdb.firebaseio.com/publish/${publishId}-${this.data.request.query.uid}.json`).then(req=>req.json());
        return fetch('https://formFacade.firebaseio.com/publish/'+publishId+'.json').then(req=>req.json());
    }
    ...
    this.fetchFacade = function(publishId)
    {
        if(this.data.request.query.template == 'cloud')
            ...
            return fetch(`https://formfacade-template-default-rtdb.firebaseio.com/facade/${publishId}-${this.data.request.query.uid}.json`).then(req=>req.json());
            
        return fetch('https://formFacade.firebaseio.com/facade/'+publishId+'-editable.json').then(req=>req.json());
    }

    this.fetchTeam = function(userId)
    {
        return fetch('https://formFacade.firebaseio.com/team/'+userId+'/info.json').then(req=>req.json());
    }

    this.fetchUser = function(userId)
    {
        return fetch('https://formFacade.firebaseio.com/user/'+userId+'/paid.json').then(req=>req.json());
    }

    this.fetchPrefill = function(publishId, prefillId)
    {
        return fetch('https://formFacade.firebaseio.com/prefill/'+publishId+'/link/'+prefillId+'.json').then(req=>req.json());
    }
    ...
    this.init = function(savedId)
    {
        ...
        var prms = [
            ...
            this.fetchTeam(userId), this.fetchUser(userId)
        ];
        ...
        if(prefillId)
        {
            prefillprm = this.fetchPrefill(publishId, prefillId).then(jso=>{
        ...
        return Promise.all(prms).then(function(rs){
            ...
            if(curr.isPaid(userId, {paid:paid}))
            {
            ...
            var query = curr.data.request.query||{};
            var ln = setting.language||info.language||query.lang;
    ...
    this.addLinkTag = function(lnkurl)
    {
        ...
        var version = this.data.devEnv?new Date().getTime():29;
    ...
    this.render = function()
    {
        ...
        if(pypanes.length>0)
        {
            ...
            window.loadingScripts = window.loadingScripts||curr.loadScripts([peerhost+'/js/pay/google-forms.js?_=v4']);
            var fac = curr.data.facade||{};
            var stg = fac.setting||{};
    ...
    this.getLocation = function(widget)
    {
        var googlemapsexists = window.google&&window.google.maps;
        var mapsrc = 'http://maps.google.com/maps/api/js?key=AIzaSyAv-ju9O47iVDb45oQsfdOc8m8JLjdWp6s&libraries=places';
        Promise.resolve(googlemapsexists||this.loadScripts([mapsrc])).then(_=>{
            navigator.geolocation.getCurrentPosition(function(position){
                ...
                geocoder.geocode({latLng}, function(results, status){
                    ...
                    {
                        var types = ["street_address", "establishment"];
                        results = results.filter(rs=>rs.types.filter(typ=>types.indexOf(typ)>=0).length>0);
                        var {formatted_address} = results[0]||{};
                        if(widget)
                        {
                            widget.value = formatted_address;
                            const options = {
                              bounds: {
                                north: latitude + 0.1, south: latitude - 0.1,
                                east: longitude + 0.1, west: longitude - 0.1,
                              },
                              fields: ["address_components", "geometry", "icon", "name"],
                              strictBounds: false, types
                            };
                            const autocomplete = new google.maps.places.Autocomplete(widget, options);
                        }
                    }
                });
            });
    ...
    this.jumptoSection = function(frm, secid, deftrg, trg, wid)
    {
        ...
        {
            loadOneTap().then(loggedin=>curr.login(loggedin))
            .catch(err=>err);
            //.catch(err=>curr.login({email:'mani.doraisamy@gmail.com', name:'Mani D', sub:'105242287939464144485'}));
        }
        this.render();
        if(wid)
        {
    ...
    this.showPeerPayment = function(secid, amt)
    {
        ...
        var scripts = [peerhost+'/js/pay/payment-forms.js?_=v5'];
...
FormFacade.prototype.calculateEngine = function(tmpl, opts={})
{
    ...
    var params = {
        ...
        SECTION:'__section__', SYMBOL:defcurrency
    };
    ...
    for(var attr in mapping)
    {
        ...
        if(itm.entry)
        {
            var entryvar = 'entry'+itm.entry;
            params[attr.split('-').join('')] = params[entryvar];
        }
    }
    ...
    params.summary = params.SUMMARY = function(pattern, all=true)
    {
        ...
        if(entr.emailAddress)
        {
            var ln = '<tr><td valign="top">Email:</td><td>'+entr.emailAddress+'</td></tr>';
            valitms.push(ln);
        }
        itms.forEach(function(item, itmi){
            var fitm = fcitms[item.id]||{};
            ...
            if(item.type=='GRID' && val)
            {
                ...
                if(valids.length>0)
                {
                    ...
                    item.rows.forEach(function(rw, r){
                        ...
                        {
                            rvals = Array.isArray(rvals)?rvals:[rvals];
                            valitms.push('<tr><td valign="top">'+rw.value+':</td><td>'+rvals.join(', ')+'</td></tr>');
                        }
                    });
                }
            }
            else if(item.type=='PARAGRAPH_TEXT' && val)
            {
                if(fitm.measure=='Configurable')
                {
                    ...
                    rws.forEach(rw=>{
                        ...
                        var ln = splt.pop();
                        var [vid, qnt] = ln.trim().split(' * ');
                        if(vid && qnt) valitms.push('<tr><td valign="top">'+splt.join(' | ')+':</td><td>'+qnt+'</td></tr>');
                    });
                }
                else
                {
                    var ln = '<tr><td valign="top">'+item.title+':</td>';
                    ln += '<td>'+val.split('\n').join('<br>')+'</td></tr>';
                    valitms.push(ln);
                }
            }
    ...
    {
    	return params.SUMMARY(true, scope||true);
    }
    ...
    {
        var itms = params.getScorable(arguments);
        var filtered = itms.filter(itm=>itm.type=='MULTIPLE_CHOICE'||itm.type=='LIST'||itm.type=='SCALE');
        if(filtered.length==0) return 0;
        var tot = params.points.apply(params, arguments)/filtered.length;
    ...
    params.editurl = params.EDITURL = function()
    {
        ...
        if(savedId)
        {
            ...
            if(shortId) url = `${currency?'neartail.com':'formfaca.de'}/rs/${shortId}/${savedId}`;
            return url;
        }
    }
